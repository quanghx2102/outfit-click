...
import { getOutfitById } from '@/server/outfits/outfit.service';
import {
  uploadMediaAsset,
...
// Uploads cover image for an outfit, updates outfit.coverImageUrl, records media_asset.
export async function POST(request: NextRequest, { params }: Params) {
  ...
  if (
    ...
    !permissions.includes(PERMISSIONS.OUTFITS_UPDATE)
  ) {
    return NextResponse.json({ error: 'Forbidden' }, { status: 403 });
  }
